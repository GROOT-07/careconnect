...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--cream)' }}>
      ...
      <nav style={{ height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px', borderBottom: '1px solid var(--light-gray)', background: 'white', position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <div className="dash-tabs" style={{ display: 'flex', gap: 2 }}>
          {tabs.map(tab => {
            ...
            return (
              <Link key={tab.href} href={tab.href} style={{ padding: '7px 14px', borderRadius: 10, fontSize: 13, fontWeight: active ? 500 : 400, color: active ? 'var(--charcoal)' : 'var(--warm-gray)', background: active ? 'var(--cream)' : 'transparent', textDecoration: 'none', transition: 'all 0.15s' }}>{tab.label}</Link>
            )
          })}
        </div>

        <div className="dash-user" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: '50%', background: cfg.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Fraunces, serif', fontSize: 13, color: 'white', fontWeight: 600 }}>{initial}</div>
          <span style={{ fontSize: 13, color: 'var(--warm-gray)' }}>{session.user?.name}</span>
          <button onClick={() => signOut({ callbackUrl: '/' })} style={{ fontSize: 12, color: 'var(--warm-gray)', background: 'none', border: '1px solid var(--light-gray)', padding: '5px 12px', borderRadius: 8, cursor: 'pointer' }}>Sign out</button>
        </div>

        <button className="dash-burger" onClick={() => setMenuOpen(o => !o)} style={{ display: 'none', flexDirection: 'column', gap: 5, background: 'none', border: 'none', cursor: 'pointer', padding: 8 }}>
      ...
      {menuOpen && (
        <div className="dash-mobile-menu" style={{ background: 'white', borderBottom: '1px solid var(--light-gray)', padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 4 }}>
          {tabs.map(tab => {
            const active = pathname === tab.href
            return (
              <Link key={tab.href} href={tab.href} onClick={() => setMenuOpen(false)} style={{ padding: '10px 14px', borderRadius: 10, fontSize: 14, fontWeight: active ? 500 : 400, color: active ? 'var(--charcoal)' : 'var(--warm-gray)', background: active ? 'var(--cream)' : 'transparent', textDecoration: 'none' }}>{tab.label}</Link>
            )
          })}
          <div style={{ borderTop: '1px solid var(--light-gray)', marginTop: 8, paddingTop: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 13, color: 'var(--warm-gray)' }}>{session.user?.name}</span>
            <button onClick={() => signOut({ callbackUrl: '/' })} style={{ fontSize: 13, color: 'var(--terra)', background: 'none', border: 'none', cursor: 'pointer' }}>Sign out</button>
          </div>
        </div>
      )}
